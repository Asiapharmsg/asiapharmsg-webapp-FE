...
import { useAddProduct, useEditProduct } from '../../hooks/products';
...
export default function _EditProduct({ product, onFinishEditing }) {
  ...
  const { mutate: editProductQuery, isLoading } = useEditProduct();
  const { data: categories } = useFetchCategories();
  const user = useSelector((state) => state.user);

  useEffect(() => {
    if (product) {
      console.log("product tier price : ", product.price_tier_1);
      setInventoryCount(product.inventory_count);
      setStatus(product.status);
      setExpiryDate(dayjs(product.expiry_date).format('YYYY-MM-DD'));
  ...
  const initialValues = {
    ...
    price_tier_1: parseFloat(product?.price_tier_1).toFixed(2) || '',
    price_tier_2: parseFloat(product?.price_tier_2).toFixed(2) || '',
    price_tier_3: parseFloat(product?.price_tier_3).toFixed(2) || '',
    remarks: product?.remarks || '',
  };

  const editProductHandler = (values, actions) => {
    console.log("edit product page / unit measurement : ", unitMeasurement);
    if (!categoryId || !expiryDate || !status) {
    ...
    editProductQuery({ newProduct: formData, productId: product?.id });
    actions.resetForm();
    onFinishEditing();
  };
  ...
  return (
    <Formik
      ...
      onSubmit={(values, actions) => {
        editProductHandler(values, actions);
      }}
    >
      {({
        values,
          ...
          <div className='account-details-form'>
            ...
            {!isLoading && (
              <form>
                ...
                <div className='single-input-item'>
                  <label htmlFor='quantity_promo' className='required'>
